    ...
    try:
        if call.message:
            if call.data == 'sklepka1':
                ...
                bot.send_message(call.message.chat.id,
                                 'Новый день - новый <b>Challenge</b>.❗️\n\nУмеешься отжиматься🔥 на руках? Если не знаешь, то самое время проверить. Подходи к стене и забрасываем ноги, что бы оказаться в стойке. Пытаемся отжаться.\n\nЕсли не можешь или чувствуешь, что слишком сложно - практикуй просто стоять в таком положении. Укрепишь плечи и подготовишься к отжиманиям.\n\nПрокачайся🚀 за карантин❗️', parse_mode = 'html')
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
