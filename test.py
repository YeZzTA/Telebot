    ...
    try:
        if call.message:
            if call.data == 'sklepka1':
                ...
                bot.send_message(call.message.chat.id,
                                 'У кого есть турник, обязательно затестите эту годноту! Ставим таймер и начинаем делать!\n\nБёрпи + подтягивания.\n\nКак это выглядит? Становишься в позицию упор лёжа, делаешь одно отжимание, выпрямляешься и делаешь 1 подтягивание. Всё это 1 повторение. Делаем таких 30 раз на время, задача закончить меньше чем за 150 секунд или 2,5 минуты.\n\nПо своему опыту вижу, что в Воркауте комбинации базовых упражнений дают максимальный результат.\n\nВсе тяжёлые элементы невозможны без достойной базовой подготовки! Не ленись сейчас и после карантина все элементы пойдут один за другим!')
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
