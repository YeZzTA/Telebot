    ...
    try:
        if call.message:
            if call.data == 'sklepka1':
                ...
                bot.send_message(call.message.chat.id,
                                 'Новый день - новый <b>Challenge</b>.❗️\n\nУже умеешь приседать на 1 ноге? И на правой и на левой? Если да, то завтра будет новый недельный 🔥челлендж, если нет, то самое время попробовать❗️\n\nСовсем скоро будет добавлена основная команда. Это - <b>База</b>🔥❗️\n\n<b>База</b> - это лучшее, что ты видел и делал, так как в неё будут добавлены тренировки на все группы мышц для начинающих и до опытных атлетов❗️\n\nСамым крутым является то, что независимо от твоего уровня, ты получишь правильную тренировку, с возможностью постоянно повышать свои результаты🚀 и изучать новые движения❗️ Скучно не будет❗️\n\nПрокачайся🚀 за карантин❗️', parse_mode = 'html')
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
